...
exp_set <- read.table("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_BL60/gene_lists/expression_geneset_106.txt")
exp_set <- exp_set$V1
...
write.table(cnv_file,"~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_BL60/7. CNV/ESCC_Biseq_CNV_seq_BL60_all.txt", quote = F,row.names = F,sep = "\t")
write.table(cnv_male,"results/BaiduII_BL60/7. CNV/ESCC_Biseq_CNV_seq_BL60_male.txt", quote = F,row.names = F,sep = "\t")
write.table(cnv_female,"results/BaiduII_BL60/7. CNV/ESCC_Biseq_CNV_seq_BL60_female.txt", quote = F,row.names = F,sep = "\t")
############# 2. Import data from GISTIC2  ###############
#### gistic score total and female and male 
scores_total <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/total/scores.gistic")
scores_female <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/female/scores.gistic")
scores_male <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/male/scores.gistic")

#### amp and del resgions female and male 
regions_male <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/female/regions_track.conf_99.bed")
regions_female <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/male/regions_track.conf_99.bed")
...
ori_total_cnv_thresholded_genes <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/total/all_thresholded.by_genes.txt")
total_cnv_thresholded_genes <- ori_total_cnv_thresholded_genes[,4:59]
...
## female
ori_female_thresholded_gene <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/female/all_thresholded.by_genes.txt")
female_thresholded_gene <- ori_female_thresholded_gene[,4:23]
...
## male
ori_male_thresholded_gene <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/male/all_thresholded.by_genes.txt")
male_thresholded_gene <- ori_male_thresholded_gene[,4:23]
...
# ## Extract amp significant regions (????? how to solve it no metadata combining and different ranges)
# annoted_female_amp <- annotateGRfromGR(regions_female_amp_grange,female_amp_grange)
# annoted_female_amp <- subsetByOverlaps(regions_female_amp_grange,female_amp_grange)
# subsetByOverlaps(female_amp_grange,regions_female_amp_grange)
# 
# female_amp_gscores <- annoted_female_amp$G.score
# female_amp_gscores
# 
# ## Extract del significant regions
# annoted_female_del <- annotateGRfromGR(regions_female_del_grange,female_del_grange)
# female_del_gscores <- annoted_female_del$G.score
# female_del_gscores
# 
# 
# total_female_scores <- union(female_amp_gscores,female_del_gscores)
...
# ## Extract amp significant regions
# annoted_male_amp <- annotateGRfromGR(regions_male_amp_grange,male_amp_grange)
# male_amp_gscores <- annoted_male_amp$G.score
# male_amp_gscores
# 
# ## Extract del significant regions
# annoted_male_del <- annotateGRfromGR(regions_male_del_grange,male_del_grange)
# male_del_gscores <- annoted_male_del$G.score
# male_del_gscores

# ## Combine
# total_male_scores <- union(male_amp_gscores,male_del_gscores)

############## 5. Extract gender only and intersection regions ########
#### 5.1 Amp ####
## female prepare
ori_female_amp_cytobands <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/female/amp_genes.conf_99.txt")
...
## male prepare
ori_male_amp_cytobands <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/male/amp_genes.conf_99.txt")
...
#### 5.2 Del ####
## female
ori_female_del_cytobands <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/female/del_genes.conf_99.txt")
...
## male
ori_male_del_cytobands <- data.table::fread("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/male/del_genes.conf_99.txt")
...
saveRDS(female_only_amp_cytoband_gene_df,"~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/female_only_amp_cytoband_gene_df.rds")
## Male
...
saveRDS(male_only_amp_cytoband_gene_df,"~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/male_only_amp_cytoband_gene_df.rds")
...
saveRDS(female_only_del_cytoband_gene_df,"~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/female_only_del_cytoband_gene_df.rds")
...
saveRDS(male_only_del_cytoband_gene_df,"~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_Bl60/7. CNV/male_only_del_cytoband_gene_df.rds")
...
### there is no sig genes in amp sad..
##### 7.2 Del ####
## use annotation from female
...
### no amp
### Del
del_locations <- del_sig_chisq_result$cytoband
chr <- paste0("chr",unique(sapply(strsplit(del_locations,"q"),"[[",1)))
chr <- unique(sapply(strsplit(chr,"p"),"[[",1))
...
del_intersection_bed <- merge(del_sig_chisq_result,del_cytoband_bed,by.x = "cytoband", by.y = "new_cytoband")
del_intersection_bed <- del_intersection_bed [,c(4,5,6,7)]
colnames(del_intersection_bed) <- c("gene", "chr","start","end")
del_intersection_bed <- del_intersection_bed[,c("chr","start","end","gene")]
del_intersection_bed$chr <- paste0("Del_",del_intersection_bed$chr)
### combine
intersection_gene_circos <- del_intersection_bed
...
############## 6. Initialize circos ##############
#pdf("results/BaiduII_BL60/cnv_circos.pdf")
...
### readRDS
final_female_track <- readRDS("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_BL60/7. CNV/final_female_track.rds")
...
### readRDS
final_male_track <- readRDS("~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_BL60/7. CNV/final_male_track.rds")
...
circos.genomicLabels(intersection_gene_circos, side = "inside",cex = 0.2,col = "yellow")

## test together
final_female_gene_circos <- female_gene_circos[female_gene_circos$gene %in% g2m_geneset,]
final_male_gene_circos <- male_gene_circos[male_gene_circos$gene %in% g2m_geneset,]

final_gene_circos <- rbind(final_male_gene_circos,final_female_gene_circos)
circos.genomicLabels(final_gene_circos, labels.column = 4, side = "inside",cex = 0.7,col = "blue")
...
dev.off()
############## 10. Save result ########
saveRDS(sig_chisq_result,"results/BaiduII_BL60/CNV_chisq_results.rds")
saveRDS(final_gene_circos,"results/BaiduII_BL60/final_gene_circos.rds")

write.csv(female_thresholded_sig_fvsm,"results/BaiduII_BL60/female_thresholded_sig_fvsm.csv",row.names = T)
write.csv(male_thresholded_sig_fvsm,"results/BaiduII_BL60/male_thresholded_sig_fvsm.csv",row.names = T)

saveRDS(final_male_track,"~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_BL60/7. CNV/final_male_track.rds")
saveRDS(final_female_track,"~/onedrive/Work/phD/phd_project/TME_gender/results/BaiduII_BL60/7. CNV/final_female_track.rds")
